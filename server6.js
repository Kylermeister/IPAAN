...
const buildLineQuery = (filters, startDate, endDate) => {
  ...
  let selectFields = ["TO_CHAR(DATE_TRUNC('month', x.date), 'YYYY-MM-DD') AS month", "AVG(x.meanthroughputmbps) AS speed"];
  let groupByFields = ["DATE_TRUNC('month', x.date)"];
  let orderByFields = ["DATE_TRUNC('month', x.date)"];
...
const buildBarQuery = (filters, startDate, endDate) => {
  ...
  let query = `select AVG(x.meanthroughputmbps)`
