...
const app = express(); //all the code that will connect it to the exact database
const pool = new Pool({
    user: 'admin', //postgres
    host: 'localhost', 
    database: 'postgres', //ipaantst
    password: 'kyler', //kyler
    port: 5433, //5432
});
const myCache = new NodeCache(); //sets a cache for the server to contain frequently used requests
...
app.post('/execute-query', validateSelectQuery, async (req, res) => {
  ...
  try {
    const result = await pool.query(sql, params); //i think this line alone is where it sends the request and then receives the result
